...
import { AlertCircle, ArrowLeft, ArrowRight, CheckCircle2, MapPin, XCircle } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea";
import { Logo } from "@/components/brand/Logo";
import { AudioRecorder } from "@/components/applicants/AudioRecorder";
...
export default function ScreeningPage({ params }: { params: Promise<{ jobId: string }> }) {
  ...
  const [answers, setAnswers] = useState<Record<string, { value: string; audioBase64?: string }>>({});
  ...
  const currentAudioBase64 = currentAnswerObj?.audioBase64;
  
  const canSubmitAnswer = currentQ?.responseType === "audio" 
    ? !!currentAudioBase64 
  ...
  const submission: Submission | null = useMemo(() => {
    ...
    const ans: Answer[] = screening.questions.map((q) => ({
      ...
      audioBase64: answers[q.id]?.audioBase64,
    }));
    return {
      id: generateId("sub"),
      jobId,
  ...
  const handleAnswerText = (val: string) => {
    if (!currentQ) return;
    setAnswers((prev) => ({ 
      ...prev, 
      [currentQ.id]: { ...prev[currentQ.id], value: val } 
    }));
  };

  const handleAnswerAudio = (base64: string) => {
    if (!currentQ) return;
    setAnswers((prev) => ({ 
      ...prev, 
      [currentQ.id]: { value: "Audio Response Recorded", audioBase64: base64 } 
    }));
  };
        ...
        <AnimatePresence mode="wait" custom={direction}>
          ...
          >
            ...
            {currentQ.responseType === "audio" ? (
              <AudioRecorder onRecordingComplete={handleAnswerAudio} />
            ) : (
              <div className="mt-5">
                ...
                <Textarea
                  id="answer"
                  value={currentAnswer}
                  onChange={(e) => handleAnswerText(e.target.value.slice(0, 500))}
